jQuery(function($) {
	...
	fooday.load_more = function() {
		$('.load-more').on('click', function(e) {
			e.preventDefault();
			var section = $(this).attr('data-section');
			console.log(section);
			$('.'+section+':hidden').slice(0, 6).slideDown();
			$('.'+section+'-2:hidden').slice(0, 6).slideDown();
			$('.'+section+'-3:hidden').slice(0, 6).slideDown();
			if ($('.'+section+':hidden').length == 0 && $('.'+section+'-2:hidden').length == 0 && $('.'+section+'-3:hidden').length == 0) {
				$(this).fadeOut('slow');
			}
		});
	}

	fooday.validate_forms = function() {
		...
		if ($("#form-newsletter").length) {
			...
			$("#form-newsletter").validate({
				...
				submitHandler: function(form) {
					$(form).ajaxSubmit({
						success: function(data) {
							$("#form-newsletter:input").attr('disabled', 'disabled');
							$("#form-newsletter").fadeTo("slow", 1, function() {
								$(this).find(':input').attr('disabled', 'disabled');
								$(this).find('label').css('cursor', 'default');
								$('#alert-success2').show();
							});
							$("#alert-success2").removeClass('hidden');
						},
						error: function() {
							$("#form-newsletter").fadeTo("slow", 1, function() {
								$('#alert-error2').show();
							});
							$("#alert-error2").removeClass('hidden');
						}
					})
				}
			})
		...
		if ($("#reservation-form").length) {
			...
			$("#reservation-form").validate({
				...
				messages: {
					...
					personas: {
						required: "Indique la cantidad de personas"
					},
					fecha: {
						required: "Ingrese la fecha de la reservación"
					},
					hora: {
						required: "Indique la hora de la reserva"
					}
				...
				submitHandler: function(form) {
					$(form).ajaxSubmit({
						success: function(data) {
							$("#reservation-form:input").attr('disabled', 'disabled');
							$("#reservation-form").fadeTo("slow", 1, function() {
								$(this).find(':input').attr('disabled', 'disabled');
								$(this).find('label').css('cursor', 'default');
								$('#alert-success3').show();
							});
							$("#alert-success3").removeClass('hidden');
						},
						error: function() {
							$("#reservation-form").fadeTo("slow", 1, function() {
								$('#alert-error2').show();
							});
							$("#alert-error3").removeClass('hidden');
						}
					})
				}
			})
